"""Rebuild decisions.json from AT2 Decisions table, mirroring build_news_json.py.
...
Output schema:
  {
    ...
    "total": int,
    "rows": [...],
    "facets": { region, status, topics, authority, subtype, jurisdiction }
  }

Each row has: id, at2_id, slug, title, summary, status, subtype, outcome, source_url,
decision_date, published_date, jurisdictions, juris_isos, iso, regions, region,
authorities, authority_types, authority, topics, archived.
"""
import collections
import json
import os
...
OUT = _ROOT / "decisions.json"
DECISIONS_TABLE = "tblzSRU6MsFJPCDka"
...
def assign_slugs(records):
    """Return {at2_id: slug}. Kebab-case from Title, dedupe with -2, -3, ..."""
    seen = {}
    out = {}
    for rec in records:
...
def to_row(rec, slug_map):
    ...
    return {
        ...
        "status": f.get("Verified Status") or None,
        "subtype": f.get("Subtype") or None,
        "outcome": f.get("Outcome") or None,
...
def main():
    print("Fetching AT2 Decisions...", file=sys.stderr)
    records = fetch_all(DECISIONS_TABLE)
    print(f"Got {len(records)} records", file=sys.stderr)

    slug_map = assign_slugs(records)
    rows = [to_row(r, slug_map) for r in records]
    rows.sort(key=lambda r: (r["decision_date"] or r["published_date"] or ""), reverse=True)
